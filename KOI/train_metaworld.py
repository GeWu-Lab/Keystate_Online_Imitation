...
class WorkspaceIL:
	def __init__(self, cfg):
		...
		with open(self.cfg.expert_dataset, 'rb') as f:
			self.expert_demo, _, _, _ = pickle.load(f)
	...
	def eval(self):
		...
		while eval_until_episode(episode):
			...
			if self.cfg.suite.name == 'metaworld':
				paths.append(np.sum(path))
				success_rate.append(time_step.observation['goal_achieved'])
		...
		with self.logger.log_and_dump_ctx(self.global_frame, ty='eval') as log:
			...
			if self.cfg.suite.name == 'metaworld':
				log("success_rate", np.mean(success_rate))
			...
			if time_step.last():
				...
				if metrics is not None:
					...
					with self.logger.log_and_dump_ctx(self.global_frame,
						...
						log('step', self.global_step)
						if 'koi' in repr(self.agent) or repr(self.agent) == 'dac':
...
@hydra.main(config_path='cfgs', config_name='metaworld_config')
def main(cfg):
	root_dir = Path.cwd()
