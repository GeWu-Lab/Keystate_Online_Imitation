...
import utils
from agent.encoder import Encoder,MultiModalEncoder
...
class KOILIBEROAgent:
	def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
		...
		# models
		if self.use_encoder:
			self.encoder = MultiModalEncoder(obs_shape).to(device)
			self.encoder_target = MultiModalEncoder(obs_shape).to(device)
			repr_dim = self.encoder.repr_dim
		else:
			repr_dim = obs_shape[0]

		self.trunk_target = nn.Sequential(
			nn.Linear(repr_dim * 2, feature_dim),
			nn.LayerNorm(feature_dim), nn.Tanh()).to(device)

		self.actor = Actor(repr_dim * 2, action_shape, feature_dim,
						   hidden_dim).to(device)

		self.critic = Critic(repr_dim * 2, action_shape, feature_dim,
							 hidden_dim).to(device)
		self.critic_target = Critic(repr_dim * 2, action_shape,
		...
		from keystate import KeystateDistribution
		self.keyidx = keyidx
		self.keyframe_extractor = KeystateDistribution(self.device)
	...
	def act(self, obs, step, eval_mode):
		img_obs = obs['pixels']	
		state_obs = obs['state']
		img_obs = torch.as_tensor(img_obs, device=self.device).float()
		state_obs = torch.as_tensor(state_obs, device=self.device).float()
  
		obs = self.encoder(img_obs.unsqueeze(0), state_obs.unsqueeze(0))
	...
	def update(self, replay_iter, expert_replay_iter, step, bc_regularize=False):
		...
		obs, state, action, reward, discount, next_obs, next_state = utils.to_torch(
			batch, self.device)
		...
		if self.use_encoder:
			# encode
			obs = self.encoder(obs, state)
			with torch.no_grad():
				next_obs = self.encoder(next_obs, next_state)

		if bc_regularize:
			batch = next(expert_replay_iter)
			obs_bc, state_bc, action_bc = utils.to_torch(batch, self.device)
			obs_bc = obs_bc.float()
			state_bc = state_bc.float()
			...
			# encode
			if bc_regularize and self.bc_weight_type=="qfilter":
				obs_qfilter = self.encoder_bc(obs_qfilter,state) if self.use_encoder else obs_qfilter
				obs_qfilter = obs_qfilter.detach()
			else:
				obs_qfilter = None
			obs_bc = self.encoder(obs_bc, state_bc) if self.use_encoder else obs_bc 
			# Detach grads
	...
	def ot_rewarder(self, observations, demos, step):
		...
		demo_obs = demos['observation']

		for idx, demo in enumerate(demo_obs):
			observations = np.array(observations)
			obs = torch.tensor(observations).to(self.device).float()
			obs = self.encoder_target.get_image_feature(obs)
   
			exp = torch.tensor(demo).to(self.device).float()

			exp = self.encoder_target.get_image_feature(exp) 
			obs = obs.detach()
			exp = exp.detach()

			if self.rewards == 'sinkhorn_cosine':
				distribution, _ = self.keyframe_extractor(self.keyidx, demo)

				cost_matrix = cosine_distance(
					obs, exp)  # Get cost matrix for samples using critic network.
				transport_plan = optimal_transport_plan(
					obs, distribution, cost_matrix, method='sinkhorn',
