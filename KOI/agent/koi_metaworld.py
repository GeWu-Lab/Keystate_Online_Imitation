...
class KOIAgent:
	...
	def act(self, obs, step, eval_mode):
		...
		obs = self.encoder(img_obs.unsqueeze(0))
	...
	def update(self, replay_iter, expert_replay_iter, step, bc_regularize=False):
		...
		if bc_regularize:
			...
			obs_bc, action_bc = batch
			obs_bc = obs_bc.cuda()
			action_bc = action_bc.cuda()
			# augment
			if self.use_encoder and self.augment:
	...
	def ot_rewarder(self, observations, demos, step):
		...
		for idx, demo in enumerate(demo_obs):
			...
			obs = self.trunk_target(self.encoder_target(obs))
			obs = obs.detach()

			exp = torch.tensor(demo).to(self.device).float()
			exp = self.trunk_target(self.encoder_target(exp))
			exp = exp.detach()

			if self.rewards == 'sinkhorn_cosine':
				distribution, _ = self.keyframe_extractor(self.keyidx, demo[:, :3])
			...
			# print("the ot reward is:", np.sum(ot_rewards))
